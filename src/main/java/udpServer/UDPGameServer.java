...
public class UDPGameServer implements Runnable {
	...
	public UDPGameServer(MessageFactory messageFactory,String name) throws SocketException, UnknownHostException {
		int count = 0;
        ...
        do
        {
            try
            {
            	socket = new DatagramSocket(10000+ count);
                foundAPort = true;
            }
            catch (Exception e)
            {
	...
	public void run() {
		...
		System.out.println("Waiting for udp packets " + socket.getLocalPort());
		while (running) {
